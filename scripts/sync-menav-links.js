...
function render(items) {
  ...
  const extra = [...groups.keys()].filter(g => !GROUP_ORDER.includes(g));
  const order = [...GROUP_ORDER, ...extra];
  return order.filter(g => groups.has(g)).map(g => {
    ...
    return `<section class="cat"><h2><span>${esc(GROUP_ICONS[g] || '🔗')}</span>${esc(g)}</h2><div class="grid">${cards}</div></section>`;
  }).join('');
}
(async () => {
  ...
  if (items.length < 5) throw new Error(`too few links: ${items.length}`);
  fs.writeFileSync('links.json', JSON.stringify({ok:true, synced_at:new Date().toISOString(), items}, null, 2));
  let html = fs.readFileSync('index.html', 'utf8');
  const block = `<!-- MENAV_LINKS_START -->\n${render(items)}\n<!-- MENAV_LINKS_END -->`;
  if (!/<!-- MENAV_LINKS_START -->[\s\S]*?<!-- MENAV_LINKS_END -->/.test(html)) throw new Error('markers not found');
  html = html.replace(/<!-- MENAV_LINKS_START -->[\s\S]*?<!-- MENAV_LINKS_END -->/, block);
